...
mkdir -p "$release_dir" "$backup_dir" "$DIST_DIR/assets"
tar xzf "$ARCHIVE" -C "$release_dir"
test -f "$release_dir/index.html"
test -d "$release_dir/assets"
cp -a "$DIST_DIR/." "$backup_dir/"
cp -a "$release_dir/assets/." "$DIST_DIR/assets/"
find "$release_dir" -maxdepth 1 -type f ! -name index.html \
  -exec cp -a {} "$DIST_DIR/" \;
install -m 0644 "$release_dir/index.html" "$DIST_DIR/index.html.next"
mv "$DIST_DIR/index.html.next" "$DIST_DIR/index.html"
