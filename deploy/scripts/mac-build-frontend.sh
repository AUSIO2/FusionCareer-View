...
TMP_DIR="${TMP_DIR:-/tmp}"
SERVE_DIR="${TMP_DIR}/fc-frontend-serve"
DIST_ARCHIVE="${SERVE_DIR}/fusioncareer-frontend-dist.tgz"
...
if [[ "$DO_BUILD" -eq 1 ]]; then
  ...
  cp "$SCRIPT_DIR/python-deploy-frontend-from-mac.sh" "$SERVE_DIR/"
  ls -lh "$DIST_ARCHIVE"
fi
...
echo "==> 启动 HTTP :$HTTP_PORT (目录 $SERVE_DIR)，绑定 0.0.0.0 ..."
cd "$SERVE_DIR"
