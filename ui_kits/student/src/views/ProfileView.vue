        ...
        <div>
          ...
          <template v-if="view==='info'">
            <div class="card card-p">
              ...
              <div v-else-if="displayPageError" role="alert" style="text-align:center;padding:2rem;color:var(--ink-3)">
                <div style="margin-bottom:.75rem">{{ displayPageError }}</div>
                <button class="btn btn-secondary btn-sm" @click="loadAll">重新加载</button>
              </div>
              <template v-else>
              <div class="grid-2">
          ...
          <template v-if="view==='resume'">
            <div class="card card-p" style="margin-bottom:1rem">
              ...
              <template v-else>
              <div v-if="!resumeFiles.length" style="text-align:center;padding:1.5rem;color:var(--ink-3)">暂无简历文件</div>
              <div v-for="(r, index) in resumeFiles" :key="r.id" class="resume-row">
                ...
                <div style="display:flex;gap:.3rem">
                  <button
                    type="button"
                    class="btn-icon btn"
                    :disabled="uploading || parsingFileId===r.id"
                    title="用此简历更新我的资料"
                    @click="parseResume(r)"
                  ><i :class="['ti', parsingFileId===r.id ? 'ti-loader-2' : 'ti-sparkles']" /></button>
          ...
          <template v-if="view==='applications'">
            <div class="card card-p">
              ...
              <div v-else-if="displayApplicationError" role="alert" style="padding:1rem;color:var(--ink-3);font-size:.8rem">
                <div style="margin-bottom:.75rem">{{ displayApplicationError }}</div>
                <button class="btn btn-secondary btn-sm" @click="loadApplications">重新加载</button>
              </div>
...
const displayApplicationError = ref('')
...
async function loadApplications() {
  ...
  try {
    ...
    apps.value = []
    displayApplicationError.value = e?.message || '加载投递记录失败'
  } finally {
...
async function loadAll() {
  ...
  try {
    ...
    displayPageError.value = e?.message || '加载失败'
  } finally {
