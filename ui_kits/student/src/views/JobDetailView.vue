<template>
  <div>
    ...
    <div class="wrap" style="padding-top:1.25rem;padding-bottom:2.5rem">
      ...
      <div v-else-if="displayJobError" role="alert" style="text-align:center;padding:4rem 0;color:var(--ink-3)">
        <div style="margin-bottom:.75rem">{{ displayJobError }}</div>
        <button class="btn btn-secondary btn-sm" @click="fetchJob">重新加载</button>
      </div>
  ...
  try {
    ...
    if (job.value && !job.value.sourceUrl) {
      ...
      try {
        ...
        submitted.value = false
        toast.error(readError?.message || '投递状态加载失败')
      }
    }
  } catch (e) {
    ...
    if (allowMockFallback) {
      ...
      displayJobError.value = e?.message || '岗位加载失败'
    }
  } finally {
    loading.value = false
  }
