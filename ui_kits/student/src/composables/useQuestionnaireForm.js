...
export async function loadMyAnswerRecord(jobPostId) {
  return apiJson(`/questionnaire/my/${jobPostId}`)
}

export async function loadResumeFileList() {
  return apiJson('/user/resume/file/list')
}
...
function parseAnswers(readValue) {
  let readAnswers
  try {
    readAnswers = typeof readValue === 'string' ? JSON.parse(readValue) : readValue
  } catch {
    throw new Error('答卷数据格式异常')
  }
  if (!Array.isArray(readAnswers)
      || readAnswers.some(readAnswer => readAnswer == null || typeof readAnswer !== 'object')) {
    throw new Error('答卷数据格式异常')
  }
  return readAnswers
}
